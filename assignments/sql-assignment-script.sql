...
###########################################################
DROP DATABASE IF EXISTS nathan_db;
...
SELECT SUM(a.balance) AS "Smiths' Total Balance"
FROM customers c
JOIN accounts_customers ac ON c.customer_id = ac.customer_id 
JOIN accounts a ON ac.account_id = a.account_id
WHERE name LIKE '%Smith'
...
# Transfer $199.99 from Jason Smith to Amanda Smith
	# Credit $199.99 from Jason Smith's first account
UPDATE accounts
SET balance = (balance - 199.99)
WHERE account_id = 900001;

	# Debit $199.99 to Amanda Smith's first account
UPDATE accounts
SET balance = (balance + 199.99)
